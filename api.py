...
def get_info_from_db(id):
    ...
    with sqlite3.connect("animal1.db") as connection:
        ...
        query = (f"""
                ...
                FROM animals_task
                JOIN appearance a on a.id = animals_task.appearance_id
                JOIN animal_breeds ab on ab.id = a.breed_id
                JOIN animal_types at on a.animal_type_id = at.id
                JOIN outcome_program op on animals_task.outcome_program_id = op.id
                JOIN outcome_date od on animals_task.outcome_date_id = od.id
